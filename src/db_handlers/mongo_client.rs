...
impl MongoClient {
    pub async fn new(app_config: &AppConfig) -> Result<Self, ErrState> {
        ...
        let client_options = ClientOptions::parse(db_addr).await.map_err(|e| {
            ErrState::new(600, "MongoDBクライアントオプションの初期化に失敗".to_string(), None) // MongoDBクライアントオプションの初期化に失敗
        })?;
        
        let client = Client::with_options(client_options).map_err(|e| {
            ErrState::new(601, "MongoDBクライアントの作成に失敗".to_string(), None) // MongoDBクライアントの作成に失敗
        })?;

        if db_name.is_empty() {
            return Err(ErrState::new(602, "データベース名が空".to_string(), None)); // データベース名が空
        }
    ...
    pub async fn d_new(&self, collection: &str, data: &Value) -> Result<String, ErrState> {
        let db_lock = self.db.lock().map_err(|e| {
            ErrState::new(603, "データベースロックの取得に失敗".to_string(), None) // データベースロックの取得に失敗
        })?;
        
        let coll = db_lock.collection::<Document>(collection);

        let bson_data = bson::to_document(data).map_err(|e| {
            ErrState::new(604, "データのBSON変換に失敗".to_string(), None) // データのBSON変換に失敗
        })?;
        
        let result = coll.insert_one(bson_data, None).await.map_err(|e| {
            ErrState::new(605, "ドキュメントの挿入に失敗".to_string(), None) // ドキュメントの挿入に失敗
        })?;
        
        if let Bson::ObjectId(oid) = result.inserted_id {
            Ok(oid.to_hex())
        } else {
            Err(ErrState::new(606, "挿入されたIDがObjectIdではない".to_string(), None)) // 挿入されたIDがObjectIdではない
        }
    }

    pub async fn d_get(&self, collection: &str, query: impl Into<Document>, fields: Option<Vec<&str>>) -> Result<Option<Document>, ErrState> {
        let db_lock = self.db.lock().map_err(|e| {
            ErrState::new(607, "データベースロックの取得に失敗".to_string(), None) // データベースロックの取得に失敗
        })?;
        ...
        let result = coll.find_one(query.into(), projection).await.map_err(|e| {
            ErrState::new(608, "ドキュメントの検索に失敗".to_string(), None) // ドキュメントの検索に失敗
        })?;
        
        if result.is_none() {
            return Err(ErrState::new(609, "ドキュメントが見つからない".to_string(), None)); // ドキュメントが見つからない
        }
    ...
    pub async fn d_fud(&self, collection: &str, query: impl Into<Document>) -> Result<Option<ObjectId>, ErrState> {
        let db_lock = self.db.lock().map_err(|e| {
            ErrState::new(610, "データベースロックの取得に失敗".to_string(), None) // データベースロックの取得に失敗
        })?;
        ...
        let result = coll.find_one(query.into(), Some(options)).await.map_err(|e| {
            ErrState::new(611, "ドキュメントの検索に失敗".to_string(), None) // ドキュメントの検索に失敗
        })?;
        ...
        if let Some(doc) = result {
            ...
            return Err(ErrState::new(612, "ドキュメントが見つからない".to_string(), None)); // ドキュメントが見つからない
        }
