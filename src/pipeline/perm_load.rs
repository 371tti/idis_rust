    ...
    async fn session_check_http(&mut self) -> Result<&mut Self, ErrState> {
        ...
        if let Some(session_vec) = &self.state.session_id {
            ...
            match self.app_set.session.user(session_vec.clone()).await {
                ...
                Ok(None) => {
                    // セッションIDが無効な場合
                    match self.app_set.session.set().await {
                        Ok(new_session_vec) => {
                            ...
                            if let Err(e) = self.app_set.user.set(&guest_user_ruid.to_u128(), &format!("@guest{}", guest_user_ruid.to_string()), &0, &vec![everyone_permission], &new_session_vec).await {
